...
const ProductCard = () => {
  ...
  const handleGetAllProducts = async () => {
    try {
      ...
      setProducts(allProducts.data.imageLinks);
    } catch (error) {
      ...
      <div className="grid grid-flow-cols grid-cols-4 gap-x-2 gap-y-6 pt-4 px-2 overflow-y-hidden">
        {productsLoading ? (
          ...
          <div className="w-full flex justify-center">
            <p className="font-Philosopher">products are on the way from database...</p>
          </div>
          </>
        ) : (
          <div className="w-full h-full">
            ...
            products.map((product, index) => (
              <div
                className="bg-slate-600 rounded-md transition-transform duration-300 hover:scale-105 hover:shadow-lg hover:cursor-pointer"
                key={index}
              >
